...
DynAccUnit::DynAccUnit (string iname, Component *parent, 
        uint32_t num_neurons, uint32_t depth, uint16_t core_idx)
    : Module ("DynAccUnitModule", iname, parent, depth)
{
    ...
    this->dyn_pipeline_depth_ = depth;
    // NBC-SRAM-NBC-(NB(depth))-AMQ-AT-AMQ
    if(dyn_pipeline_depth_ > 15)
        SIM_ERROR ("Max pipeline depth is 15", 
                GetFullName().c_str());
    
    /* variable initialization */
    this->acc_pipeline_depth_ = acc_depth;
    acc_state_ = 0;
    acc_mask_ = (1 << (acc_pipeline_depth_)) - 2;
    sent_accfin_ = true;

    pipe_state_ = 0;
    pipe_mask_ = (1 << (dyn_pipeline_depth_-2)) - 2;
    
    spike_state_ = 0;
    spike_mask_ = (1 << dyn_pipeline_depth_) - 2;
    is_idle_ = true;
    is_finish_ = true;
...
void DynAccUnit::Operation (Message **inmsgs, Message **outmsgs, 
        Instruction *instr)
{
